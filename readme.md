...
**This project is work in progress - there is nothing to use yet unless you wish to get involved with the ongoing development of this project.**

Project mediaswitch is designed to be a scalable VOIP switch. Forget Swiss-army knife supporting 20 protocols bridging old ISDN routes with H323 or SIP. This project will be designed to be slim, efficient and fast using an event-driven, asynchronous architecture. SIP, call control and RTP.

The design will be broken down into 3 core components. Each component is primarily a single threaded architecture with I/O completion ports for network (think Nginx over Apache). This is to:
...
Each component can run either on the same server or on separate servers. This allows, for example, server 1 running SIP and Call control, then if you heavily transcode which increases the load of the RTP servers, these can be multiplied up by running RTP on 5 other servers.
...
Like project-sip, this has a http interface for other components to talk to it with, however, part of teh Node JS library it will simplify writing call control scripts. For example:
