        ...
        else if( laststate.answered && c.answered && !c.hungup )
        {
          ...
          if( "refer" in c.info )
          {
            if( "replaces" in c.info.refer )
            {
              ...
              {
                leg1 = this.calls[ c.info.refer.replaces ].other
              }
              /* Attended xfer - not linked - we have a call reference to it */
              var other = c.other
              if( false === other )
              {
console.log("one legged")
                ...
                /*
                  ...
                  so we place that channel on leg 3 as le 3 will be ended.
                */
                var channeltohangup = leg1.metadata.channel
                var sdptohangup = leg1.info.sdp.us
                
                leg1.info.sdp.us = c.info.sdp.us
                leg1.info.sdp.us.m[ 0 ].direction = "sendrecv"
                leg1.metadata.channel = c.metadata.channel

                c.info.sdp.us = sdptohangup
                c.metadata.channel = channeltohangup
              ...
              {
                /* we are xfering other to c.info.refer.replaces - I think! */
                c.unlink()
                /* We can only xfer a call if there is an other */
                c.unmix( () =>
                {
                  if( c.info.refer.replaces in this.calls )
                  {
                    var aleg = this.calls[ c.info.refer.replaces ]
                    aleg.unlink()
                    aleg.unmix( () =>
                    {
                      c.mix( aleg )
            ...
            else
            {
              ...
              if( false !== other )
              {
                ...
                c.unmix( () =>
                {
                  ...
                  c.unlink()
                  this.sipserver( { "callid": c.callinfo.callid }, "/dialog/notify" )
                } )
              }
            }
          }
