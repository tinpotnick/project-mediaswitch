  ...
  constructor( control, callinfo )
  {
    ...
    this.onhold = () =>
    {
      if( "channel" in this.metadata )
      {
        ...
        if( "parent" in this.metadata.family )
        {
          other = this.metadata.family.parent
        }
        else
        {
          for ( let child of this.metadata.family.children )
        ...
        if( false !== other )
        {
          ...
          {
            if( "channel" in other.metadata )
            {
              this.control.server( this.moh, 
                                    "/channel/" + other.metadata.channel.uuid + "/play", 
                                    "PUT", 
                                    this.metadata.channel.control, 
                                    ( response ) =>
              {
                this.callinfo.sdp.us.m[ 0 ].direction = "inactive"
                this.postrequest( "ok", { "sdp": this.callinfo.sdp.us } )
              } )
            }
          } )
        }
